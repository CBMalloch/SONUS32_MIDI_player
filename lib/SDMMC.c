...
void initSD( void)
{
  ...
  SDCS_TRIS = 0;      // make Card select an output pin

  SDI2Rbits.SDI2R = SDI_PPS_ITM;   // PPS for MISO
  SDO_REG = SDO_PPS_ITM;           // PPS for MOSI

  
  // init the spi module for a slow (safe) clock speed first
  SPI2CON = 0x8120;   // ON, 8-bit master, CKE=1; CKP=0, sample middle
  // SPI2BRG = 71;       // clock = Fpb/144 = 250kHz
  #define desired_SPI_freq   250000
  SPI2BRG = FPB / (2 * desired_SPI_freq) - 1;
  // s.b. 8e6 / (2 * 2.5e5) - 1 = 16 - 1 = 15`
...
// for additional consideration, see p.190 in The C Book
{
    int i, r;
